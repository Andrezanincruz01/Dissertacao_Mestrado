# -*- coding: utf-8 -*-
"""Penalização ALG. 7.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1PsH9kWc0QD_AGZ2teWP-Q19xxPQzbryi
"""

#===============================================
# Código - CASO SIMPLES - 01/07 - AL-Quad
#===============================================
# Aplicação do Algoritmo 7 no problema de portfólio
# Caso Simples com F(x)+\tau||xoy||²
...
def F_mobj(z):
    """
    Retorna F(z)=(0.5x^TQx,-vx)
    """
    x = z[:n]
    return np.array([0.5 * x@(Q@x), -v@x])

def f_penalizado(z, tau):
    x, y = z[:n], z[n:]
    base = F_mobj(z)
    penalty = tau * np.linalg.norm(x * y)**2
    return base + penalty * np.ones_like(base)


def jacobiano_f_penalizado(z, tau):
    """
    Retorna uma matriz 2×(2n) onde cada linha i é o gradiente de L em relação a [x;y].
    """
    x, y = z[:n], z[n:]
    # Gradiente de F_mobj
    grad1_x = Q @ x
    grad2_x = -v
    # Termo de penalidade
    term = 2 * tau * (x * y)  # Derivada de ||xoy||²
    # Gradientes do termo de penalidade
    pen_x = term * y
    pen_y = term * x
    # Monta as duas linhas do gradiente
...
def Armijo_tamanho_passo(z, d, F, J, A_k, delta):
    ...
    F : Função objetivo penalizada (na prática "f_penalizado"), que retorna vetor de dimensão 2.
    J : Avaliação do gradiente em z na direção d, isto é [GradL_1d, GradL_2d].
...
def processo_interno_PGMMOP(z, tau, alpha, max_iter=50,M=10, delta=0.5, tol_inner=1e-08):
    """
    Usado para executar ométodo PGMMOP para o f_penalizado.
    Parâmetros:
    z : Ponto inicial [x,y].
    ...
    Fzp :Valor de f_pen(z) durante o processo.
    ...
    for k in range(max_iter):
        Fzp = f_penalizado(z, tau)
        f_vals.append(Fzp)
        d0 = np.zeros_like(z)

      # Definimos os vetores gradientes em relação a x e y da função lagrangiana:
        x, y = z[:n], z[n:]
        b1 = np.concatenate([Q @ x + tau * (x * (y**2)), tau * ((x**2) * y)])
        b2 = np.concatenate([-v + tau * (x * (y**2)), tau * ((x**2) * y)])
        ...
        Jv = np.array([
            jacobiano_f_penalizado(z,tau)[0]@d,
            jacobiano_f_penalizado(z,tau)[1]@d])
        a = Armijo_tamanho_passo(z, d, lambda zz: f_penalizado(zz, tau),
                                      Jv, Ak, delta)
        z_new = z + a*d
...
def processo_externo_penalizacao(z0, alpha, max_outer=50, tol_stop=1e-8):
    """
    Executa o loop externo de atualização de τ e λ para o f penalizado.
    ...
    for k in range(max_outer):
        ...
        z_new, d, it_int, Fz = processo_interno_PGMMOP(z, tau, alpha)
        t1 = time.time()
        t_int += (t1 - t0)

        x_new, y_new = z_new[:n], z_new[n:]
        norm_diff = np.linalg.norm(Fz - F_prev)
        prod = np.dot((z_new[:6]**2)/(np.linalg.norm(z_new[:6])**2 + 1e-12),
                      (z_new[6:]**2)/(np.linalg.norm(z_new[6:])**2 + 1e-12))

        print(f"Iter. ex: {k} (it. int = {it_int}, ||d|| = {np.linalg.norm(d):.2e}): "
              f"tau = {tau:.7f}, x²/||x||²oy²/||y²|| = {prod:.2e}, ||ΔF|| = {norm_diff:.2e}")
        ...
        if cond1 and cond2:
            ...
            return z_new, d, k+1, Fz, tau, norm_diff, t_ext, t_int
        ...
        F_prev, normHad_prev, z = Fz, prod, z_new
        t_ext += time.time() - t0

    # Caso atinja o máximo de iterações programadas, retorna o seguinte
    return z, d, max_outer, Fz, tau, norm_diff, t_ext, t_int
...
# Listas temporárias para acumular os resultados de cada start
    ...
    for i, z0 in enumerate(proj_inits, 1): #(i=1,2,3,...) para não começar i=0
        ...
        (z_opt, d, it_ex, Fz, tau_f,
         diff_f, t_ext, t_int) = processo_externo_penalizacao(z0, alpha)
...
for alpha in alpha_values:
    ...
    plt.scatter([],[],color=colors[alpha],s=30,marker='o',label=fr'AL-Quad α$=${alpha}')
plt.xlabel('Risco',fontsize=16)
plt.ylabel('Retorno',fontsize=16)
...
for alpha in alpha_values:
    ...
    rets_al   =   (pts_al @ v)
    plt.scatter(riscos_al, rets_al,marker='o',color=colors[alpha],s=60,label=f'AL-Quad α$=${alpha}')
...
plt.title('Risco vs Retorno - AL-Quad vs MIQP')
plt.xlabel('Risco',fontsize=16)
plt.ylabel('Retorno',fontsize=16)
plt.legend()
plt.grid(True)
...
#===============================================
# Código - Port 1 - AL-Quad
# Aplicação do Algoritmo 7 no problema de portfólio
# Port 1 com F(x)+\tau ||xoy||²
#===============================================
...
def jacobiano_f_penalizado(z, tau):
    """
    Retorna uma matriz 2x(2n) onde cada linha i é o gradiente de L em relação a [x;y].
    """
    x, y = z[:n], z[n:]
    # Gradiente de F_mobj
    grad1_x = Q @ x
...
def processo_externo_penalizacao(z0, alpha, max_outer=50, tol_stop=1e-8):
    ...
    for k in range(max_outer):
        ...
        x_new, y_new = z_new[:n], z_new[n:]
        norm_diff = np.linalg.norm(Fz - F_prev)
        prod = np.dot((z_new[:31]**2)/(np.linalg.norm(z_new[:31])**2 + 1e-12),
                      (z_new[31:]**2)/(np.linalg.norm(z_new[31:])**2 + 1e-12))
