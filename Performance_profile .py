...
import pygmo as pg          # ferramentas de otimização (usado aqui para hipervolume)
...
def hv_from_nd(nd_pts, ref):
    """
    Função para calcular o Hipervolume
    Se a frente está vazia, devolve a penalidade BIG.
    Do contrário, usa pg.hypervolume(nd) e usa compute(ref).
    pg.hypervolume calcula a medida de Lebesgue da área dominada pelos pontos ND e limitada
    pelo ponto de referência ref.
    """
    if nd_pts.size == 0:# o algoritmo não devolveu frente ND
        return BIG                   # nenhuma solução => custo máximo
    return pg.hypervolume(nd_pts).compute(ref)
...
for n in NS:  #repetimos todo o processo para cada instância do problema n=85,89,98
  #  αs = conjunto de valores de cardinalidade testados naquele problema
    ...
    salva_metricas= {'hv_global': hv_global,
             'gamma_global': gamma_global}

    # 2-C) loop das três métricas
 ...
 #                                     'exp' : [ valor_α_1, valor_α_2, ..., valor_α_m ]}
        ...
        # Teremos cada experimento s corespondendo a uma coluna em perf[pen], onde cada coluna representa um alpha no problema de dimensão n.
        ...
        for i in range(len(αs)): #Vai percorrer a coluna i=0,...,m-1, onde cada coluna corresponde ao valor de um alpha específico.
            ...
            for pen in PENS:
                v = perf[pen][i]                  # valor da metrica para o método "pen" para o atual alpha (coluna i).
                if best == 0.0:                              # Caso degenerado (Por exemplo,HV=0)
                    ratios[pen].append(1.0 if v == 0.0 else BIG)
                else:
                    ratios[pen].append(v/best) # assim formamos para cada método a lista r_{p,s} >= 1
...
def frente_AL(metodo):
    pts = conv[f'convergence_{metodo}_n{n_fig}']['nd_points'].item().get(alpha,
                                                                         np.empty((0,2))) #.npz com todas as execuções
...
nd = [] # lista vazia para guardar pontos não dominados
for s in sols:
    if not any((o['f1'] <= s['f1'] and o['f2'] <= s['f2']) and
               (o['f1'] < s['f1'] or o['f2'] < s['f2']) for o in sols if o is not s):
        nd.append(s)
...
ref = global_referencia(n_fig)             # (½σ²_max , −ret_min)

hv_miqp = pg.hypervolume(pts_miqp).compute(ref) # Hipervolume MIQP
...
for pen in ['quad', 'exp']:
    ...
    hv_al  = pg.hypervolume(pts_al).compute(ref)
    cover  = 100 * hv_al / hv_miqp

    print(f"AL-{pen:<4s} cobre {cover:5.1f}% do hipervolume MIQP "
