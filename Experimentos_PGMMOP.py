...
#
# Use MathText sem LaTeX externo, mas com Computer Modern
...
#========================================
np.random.seed(31) #Semente de geração
def F(x):
    """
    Função principal (Das e Dennis – não convexa)
    Entrada: x em R^5
    Saída: [f1, f2]
      f1 = x[0]² + x[1]² + x[2]³ + x[3]² + x[4]^5
...
def buscadirecao(x, bounds, tol_slsqp=1e-10):
    """
    Resolve o subproblema de direção via SLSQP (problema suave com variável auxiliar t).

    Parâmetros
    x = Ponto atual no espaço das variáveis.
    bounds = Caixa com limites (lower, upper) para cada variável.
    tol_slsqp = Tolerância de parada do solver SLSQP
    Retorna
    d_opt = Direção de descida encontrada pelo subproblema.
    t_opt = Valor da variável auxiliar t na solução ótima.
    phi =Valor da função objetivo do subproblema na solução ótima.
    """
...
for i, x0 in enumerate(initial_points):
    ...
    print("Imagem da solução ótima F(x*):", np.float64(Fx))
...
def TRIDIAgrad(x):
    """
    Retorna (gradf1, gradf2, gradf3), cada qual em R³.
    """
...
def buscadirecao(x, bounds, tol_slsqp=1e-10):
    ...
    G          = np.vstack([g1, g2, g3])       # matriz 3x3
    ...
    # restrições da caixa x+d pertence a bounds
    for i, (lo, up) in enumerate(bounds):
...
# ==========================================================
# Geração de amostras para o espaço dos objetivos
# ==========================================================
...
# ==========================================================
#  Separação de soluções dominadas / não-dominadas
# ==========================================================
...
#  Configurações de fonte (MathText => Computer Modern)
plt.rcParams['text.usetex']      = False
plt.rcParams['font.family']      = 'serif'
plt.rcParams['font.serif']       = ['Computer Modern']
plt.rcParams['mathtext.fontset'] = 'cm'
...
def compute_A_k(valores_F, k, M=10):
    ...
    if not recentes:
        return valores_F[-1]
    A_1= max([p[0] for p in recentes])
    A_2= max([p[1] for p in recentes])
    A_3= max([p[2] for p in recentes])
    return (A_1,A_2,A_3)
...
def Armijo_tamanho_passo(z, d, F, J, A_k, delta):
    ...
    for j in range(30):
        ...
        right_side = np.array(A_k) + delta*t_val*J
        if np.all(F_next <= right_side):
            return t_val
    return 1
...
for i, sol in enumerate(imagens_sol):
    ...
    if dominado:
        ...
        print(f"Solução {i+1} com imagem F(x*) = {np.float64(sol)} é DOMINADA entre os pontos ótimos.")
    else:
        nao_dominadas_otimas.append(sol)
        print(f"Solução {i+1} com imagem F(x*) = {np.float64(sol)} NÃO é dominada entre os pontos ótimos.")
...
for sol in nao_dominadas_otimas:
    ...
    if dominada:
        ...
        print(f"Solução F(x*) = {np.float64(sol)} foi DOMINADA pelo espaço de amostras.")
    else:
        nao_dominadas_finais.append(sol)
        print(f"Solução F(x*) = {np.float64(sol)} NÃO foi dominada pelo espaço de amostras.")
...
def F(x):
    """
    f1(x) = 1 − exp(−[(x_1−1/sqrt(2))² + (x_2−1/sqrt(2))²])
    f2(x) = 1 − exp(−[(x_1+1/sqrt(2))² + (x_2+1/sqrt(2))²])
    """
    s = 1/np.sqrt(2)
    t1 = (x[0] - s)**2 + (x[1] - s)**2
    t2 = (x[0] + s)**2 + (x[1] + s)**2
...
def PGMMOP(x0, max_iter=50, M=10, delta=0.5, tol=1e-8):
    ...
    for k in range(max_iter):
        ...
        t = Armijo_tamanho_passo(x, d, F, slope, A_k,delta)
        x = proj(x + t*d)
...
for i in range(n_starts):
    ...
    print("Iterações:", iters, " φ final:", phi_final, " F(x*):", np.float64(Fx))
